...
def load_model_frame(
    ...
    *,
    ...
    """Load a final Etna CSV as a numeric, time-indexed modelling dataframe."""
    df = pd.read_csv(csv_path, parse_dates=[time_col])

    if include_columns is not None:
        keep = [time_col, *include_columns]
...
def target_extremes(df: pd.DataFrame, effect: str, n: int = 10) -> pd.DataFrame:
    """Return the largest absolute effect values; useful for detecting scaling/outlier issues."""
    if effect not in df.columns:
...
def split_diagnostics(
    ...
    *,
    ...
    """Inspect the paper-style split used by find_increase_split()."""
...
def select_lag_by_var(
    ...
    *,
    ...
    """Select a VAR lag. For scaled Etna variables, keep distribution='gaussian'."""
...
def make_cause_trigger_config(
    ...
    *,
    ...
    lag = workflow.manual_lag if lag is None else int(lag)
    ...
    if parameter_source is None:
        ...
        parameter_source = f"{backend}_lag{lag}_{distribution}_{suffix}"
...
def plot_effect_with_split(
    ...
    *,
    ...
    """Plot an effect series with detected split and optional known event time."""
    ...
    plt.title(title or f"{effect}: detected split vs known event")
    plt.xlabel("Time")
