...
def pre_case_reference_interval(
    ...
    *,
    ...
    if coverage < min_coverage:
        raise ValueError(
            f"Insufficient Etna reference coverage: "
...
def prepare_case_frames(
    ...
    if raw_case.isna().any().any():
        raise ValueError("Etna case interval contains missing values.")
    if reference_raw.empty:
        raise ValueError("Etna reference interval is empty.")
    if reference_raw.isna().any().any():
        raise ValueError("Etna reference interval contains missing values.")
...
def plot_effect_with_split(
    ...
    *,
    ...
    ax.set(
        title=title or f"{effect}: automatic split with external event shown for context",
        xlabel="Time",
        ylabel="Scaled value",
    )
    ax.legend()
